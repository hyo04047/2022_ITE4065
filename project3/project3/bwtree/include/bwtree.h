...
#define GET_DEPTH_FROM_NODE(p)                (((unsigned long long)p & 0xFFFF000000000000) >> 48)
#define SET_DEPTH_AT_NODE(p, d)               (BaseNode*)((unsigned long long)p | (((unsigned long long)d) << 48))
#define REFERENCE(p)                          (BaseNode*)((unsigned long long)p & 0x0000FFFFFFFFFFFF)
...
#define InnerInlineAllocateOfType(T, node_p, ...)                                                         \
  (static_cast<T *>(new (ElasticNode<KeyNodeIDPair>::InlineAllocate(&node_p->GetLowKeyPair(), sizeof(T))) \
                        T{__VA_ARGS__}))
...
#define LeafInlineAllocateOfType(T, node_p, ...)                                                         \
  (static_cast<T *>(new (ElasticNode<KeyValuePair>::InlineAllocate(&node_p->GetLowKeyPair(), sizeof(T))) \
                        T{__VA_ARGS__}))
...
class BwTreeBase {
 public:
  ...
  class GarbageNode {
   public:
     ...
     * Constructor
     */
    NO_ASAN GarbageNode(uint64_t p_delete_epoch, void *p_node_p)
        : delete_epoch{p_delete_epoch}, node_p{p_node_p}, next_p{nullptr} {}

    NO_ASAN GarbageNode() : delete_epoch{0UL}, node_p{nullptr}, next_p{nullptr} {}
  };
  ...
  class GCMetaData {
   public:
     ...
     * Default constructor
     */
    NO_ASAN GCMetaData() : last_active_epoch{0UL}, header{}, last_p{&header}, node_count{0UL} {}
  };

  // Make sure class Data does not exceed one cache line
  static_assert(sizeof(GCMetaData) < CACHE_LINE_SIZE, "class Data size exceeds cache line length!");
 ...
 protected:
  ...
  NO_ASAN void SetLeafDeltaChainLengthThreshold(int leaf_delta_chain_length_threshold) {
  ...
  NO_ASAN void SetInnerNodeSizeUpperThreshold(int inner_node_size_upper_threshold) {
  ...
  NO_ASAN void SetInnerNodeSizeLowerThreshold(int inner_node_size_lower_threshold) {
  ...
  NO_ASAN void SetLeafNodeSizeUpperThreshold(int leaf_node_size_upper_threshold) {
  ...
  NO_ASAN void SetLeafNodeSizeLowerThreshold(int leaf_node_size_lower_threshold) {
 ...
 public:
   ...
   */
  NO_ASAN void PrepareThreadLocal() {
    INDEX_LOG_TRACE("Preparing %lu thread local slots", thread_num);
   ...
   */
  NO_ASAN void SetThreadNum(size_t p_thread_num) { thread_num = p_thread_num; }

 public:
  /*
   * Constructor - Initialize GC data structure
   */
  NO_ASAN BwTreeBase() : gc_metadata_p{nullptr}, original_p{nullptr}, thread_num{total_thread_num.load()}, epoch{0UL} {
  ...
  NO_ASAN ~BwTreeBase() {
    ...
    INDEX_LOG_TRACE("Finished destroying class BwTreeBase");
  }
   ...
   *                  BwTree is serving
   */
  NO_ASAN inline size_t GetThreadNum() { return thread_num; }
   ...
   */
  NO_ASAN inline void AssignGCID(int p_gc_id) { gc_id = p_gc_id; }
   ...
   */
  NO_ASAN static void RegisterThread() { gc_id = total_thread_num.fetch_add(1); }
   ...
   */
  NO_ASAN inline void IncreaseEpoch() { epoch++; }
   ...
   *                      for GC
   */
  NO_ASAN inline void UnregisterThread(int thread_id) {
   ...
   */
  NO_ASAN inline uint64_t GetGlobalEpoch() { return epoch; }
   ...
   */
  NO_ASAN inline GCMetaData *GetCurrentGCMetaData() { return GetGCMetaData(gc_id); }
...
class BwTree : public BwTreeBase {
 public:
  ...
  using KeyNodeIDPairBloomFilter =
      bwtree::BloomFilter<KeyNodeIDPair, KeyNodeIDPairEqualityChecker, KeyNodeIDPairHashFunc>;
  ...
  class KeyNodeIDPairComparator {
   public:
     ...
     */
    NO_ASAN KeyNodeIDPairComparator() = delete;

    /*
     * Constructor - Initialize a key-NodeID pair comparator using
     *               wrapped key comparator
     */
    NO_ASAN KeyNodeIDPairComparator(BwTree *p_tree_p) : key_cmp_obj_p{&p_tree_p->key_cmp_obj} {}
  ...
  class KeyNodeIDPairEqualityChecker {
   public:
     ...
     * Default constructor - deleted
     */
    NO_ASAN KeyNodeIDPairEqualityChecker() = delete;

    /*
     * Constructor - Initialize a key node pair eq checker
     */
    NO_ASAN KeyNodeIDPairEqualityChecker(BwTree *p_tree_p) : key_eq_obj_p{&p_tree_p->key_eq_obj} {}
  ...
  class KeyNodeIDPairHashFunc {
   public:
     ...
     * Default constructor - deleted
     */
    NO_ASAN KeyNodeIDPairHashFunc() = delete;

    /*
     * Constructor - Initialize a key value pair hash function
     */
    NO_ASAN KeyNodeIDPairHashFunc(BwTree *p_tree_p) : key_hash_obj_p{&p_tree_p->key_hash_obj} {}
  ...
  class KeyValuePairComparator {
   public:
     ...
     * Default constructor - deleted
     */
    NO_ASAN KeyValuePairComparator() = delete;

    /*
     * Constructor
     */
    NO_ASAN KeyValuePairComparator(BwTree *p_tree_p) : key_cmp_obj_p{&p_tree_p->key_cmp_obj} {}
  ...
  class KeyValuePairHashFunc {
   public:
     ...
     * Default constructor - deleted
     */
    NO_ASAN KeyValuePairHashFunc() = delete;

    /*
     * Constructor - Initialize a key value pair hash function
     */
    NO_ASAN KeyValuePairHashFunc(BwTree *p_tree_p)
        : key_hash_obj_p{&p_tree_p->key_hash_obj}, value_hash_obj_p{&p_tree_p->value_hash_obj} {}
  ...
  class AllocationMeta {
   ...
   public:
     ...
     * Constructor
     */
    NO_ASAN AllocationMeta(char *p_tail, char *p_limit) : tail{p_tail}, limit{p_limit}, next{nullptr} {}
     ...
     */
    NO_ASAN void *TryAllocate(size_t size) {
     ...
     */
    NO_ASAN AllocationMeta *GrowChunk() {
      ...
      auto *new_chunk = new char[CHUNK_SIZE()];
      AllocationMeta *expected = nullptr;

      // Prepare the new chunk's metadata field
      auto *new_meta_base = reinterpret_cast<AllocationMeta *>(new_chunk);
      ...
      new (new_meta_base) AllocationMeta{new_chunk + CHUNK_SIZE(),             // tail
     ...
     */
    NO_ASAN void *Allocate(size_t size) {
  ...
  class InnerNode : public ElasticNode<KeyNodeIDPair> {
   public:
     ...
     */
    NO_ASAN ~InnerNode() { this->~ElasticNode<KeyNodeIDPair>(); }
  ...
  class LeafNode : public ElasticNode<KeyValuePair> {
   public:
     ...
     */
    NO_ASAN ~LeafNode() { this->~ElasticNode<KeyValuePair>(); }
 ...
 public:
   ...
   */
  NO_ASAN BwTree(bool start_gc_thread = true, KeyComparator p_key_cmp_obj = KeyComparator{},
      ...
      : BwTreeBase(),
        ...
        consol_failure_cnt{0},
        insert_failure_cnt{0},
        delete_failure_cnt{0},
        installnode_failure_cnt{0},
        adjustnode_failure_cnt{0},
        finishSMO_failure_cnt{0},
    ...
    INDEX_LOG_TRACE("sizeof(KeyType) = %lu is the size of key", sizeof(KeyType));

    // We could choose not to start GC thread inside the BwTree
    // in that case GC must be done by calling the interface
    if (start_gc_thread) {
      INDEX_LOG_TRACE("Starting epoch manager thread...");
   ...
   */
  NO_ASAN ~BwTree() {
    INDEX_LOG_TRACE("Next node ID at exit: %" PRIu64 "", next_unused_node_id.load());
    INDEX_LOG_TRACE("Destructor: Free tree nodes");
    ...
    (void)node_count;
    INDEX_LOG_TRACE("Freed %lu tree nodes", node_count);
  }
  ...
  NO_ASAN void UpdateThreadLocal(size_t p_thread_num) {
    INDEX_LOG_TRACE("Updating thread-local array to length %lu......", p_thread_num);
  ...
  NO_ASAN size_t FreeNodeByNodeID(NodeID node_id) {
    ...
    retry_table[node_id] = nullptr;
    SMO_table[node_id] = false;
    ...
    while (1) {
      ...
      switch (type) {
        ...
        case NodeType::InnerInsertType:
          ...
          freed_count += FreeNodeByNodeID(((InnerInsertNode *)node_p)->item.second);
        ...
        case NodeType::InnerDeleteType:
          ...
          retry_table[((InnerDeleteNode *)node_p)->item.second] = nullptr;
        ...
        case NodeType::InnerMergeType:
          freed_count += FreeNodeByPointer(((InnerMergeNode *)node_p)->child_node_p);
          freed_count += FreeNodeByPointer(((InnerMergeNode *)node_p)->right_merge_p);
  ...
  NO_ASAN void InitNodeLayout() {
    ...
    InnerNode *root_node_p =
        reinterpret_cast<InnerNode *>(ElasticNode<KeyNodeIDPair>::Get(1, NodeType::InnerType, 0, 1,
                                                                      first_sep,  // Copy this as the first key
                                                                      std::make_pair(KeyType{}, INVALID_NODE_ID)));

    root_node_p->PushBack(first_sep);

    INDEX_LOG_TRACE("root id = %" PRIu64 "; first leaf id = %" PRIu64 "", root_id.load(), first_leaf_id);

    InstallNewNode(root_id, root_node_p);

    // Initially there is no element inside the leaf node so we set element
    // count to be 0
    LeafNode *left_most_leaf = reinterpret_cast<LeafNode *>(
        ElasticNode<KeyValuePair>::Get(0, NodeType::LeafType, 0, 0, std::make_pair(KeyType{}, INVALID_NODE_ID),
                                       std::make_pair(KeyType{}, INVALID_NODE_ID)));
  ...
  NO_ASAN void InitMappingTable() {
    ...
    INDEX_LOG_TRACE("Mapping table allocated via mmap()");
    ...
    retry_table = (std::atomic<const BaseNode *> *)mmap(nullptr, sizeof(BaseNode *) * MAPPING_TABLE_SIZE,
                                                          PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    // If allocation fails, we throw an error because this is uncoverable
    // The upper level functions should either catch this exception
    // and then use another index instead, or simply kill the system
    if (retry_table == (void *)-1) {
      INDEX_LOG_ERROR("Failed to initialize retry table");
      //      throw IndexException("mmap() failed to initialize mapping table for Bw-Tree");
      // TODO(Matt): fix this
    }

    INDEX_LOG_TRACE("Retry table allocated via mmap()");

    INDEX_LOG_TRACE("Initializing retry table.... size = %lu", MAPPING_TABLE_SIZE);
    INDEX_LOG_TRACE("Fast initialization: Do not set to zero");

    SMO_table = (std::atomic<bool> *)mmap(nullptr, sizeof(bool) * MAPPING_TABLE_SIZE,
                                                          PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    // If allocation fails, we throw an error because this is uncoverable
    // The upper level functions should either catch this exception
    // and then use another index instead, or simply kill the system
    ...
    INDEX_LOG_TRACE("Initializing SMO table.... size = %lu", MAPPING_TABLE_SIZE);
    INDEX_LOG_TRACE("Fast initialization: Do not set to zero");
  }
   ...
   */
  NO_ASAN inline NodeID GetNextNodeID() {
...
// they could call CAS and change the key
  ...
  NO_ASAN inline bool InstallNodeToRetry(NodeID node_id, const BaseNode *node_p, const BaseNode *prev_p) {
    // Make sure node id is valid and does not exceed maximum
    NOISEPAGE_ASSERT(node_id != INVALID_NODE_ID, "Node count exceeded maximum.");
    NOISEPAGE_ASSERT(node_id < MAPPING_TABLE_SIZE, "Node count exceeded maximum.");
...
// they could call CAS and change the key
    ...
    return retry_table[node_id].compare_exchange_strong(prev_p, node_p);
  }
   ...
   */
  NO_ASAN inline bool InstallRootNode(NodeID old_root_node_id, NodeID new_root_node_id) {
   ...
   */
  NO_ASAN inline void InstallNewNode(NodeID node_id, const BaseNode *node_p) { 
  ...
  NO_ASAN inline const BaseNode *GetRecycleNode(const NodeID node_id) {
    NOISEPAGE_ASSERT(node_id != INVALID_NODE_ID, "Node id out of range.");
    NOISEPAGE_ASSERT(node_id < MAPPING_TABLE_SIZE, "Node id out of range.");

    return retry_table[node_id].load();
  } 
  ...
  retry_traverse:
    ...
    INDEX_LOG_TRACE("Successfully loading root node ID");
    ...
    INDEX_LOG_TRACE("Found leaf node. Abort count = %d, level = %d", context_p->abort_counter,
   ...
   * setting to true.
   */
  NO_ASAN void NavigateSiblingChain(Context *context_p) {
    do {
      ...
      if ((node_p->GetNextNodeID() != INVALID_NODE_ID) &&
        ...
        INDEX_LOG_TRACE("Bounds checking failed (id = %" PRIu64
                        ") - "
                        "Go right.",
                        snapshot_p->node_id);
        ...
        if (context_p->abort_flag) {
          INDEX_LOG_TRACE("JumpToNodeID aborts(). ABORT");
  ...
  NO_ASAN void NavigateSiblingChainBI(Context *context_p) {
    while (1) {
      ...
      if ((node_p->GetNextNodeID() != INVALID_NODE_ID) &&
        ...
        JumpToNodeID(node_p->GetNextNodeID(), context_p);
        if (context_p->abort_flag) {
          INDEX_LOG_TRACE("JumpToNodeID() aborts for BI. ABORT");
          return;
        }
      } else {
        break;
  ...
  NO_ASAN inline NodeID LocateSeparatorByKey(const KeyType &search_key, const InnerNode *inner_node_p,
    ...
    auto it =
        std::upper_bound(start_p, end_p, std::make_pair(search_key, INVALID_NODE_ID), key_node_id_pair_cmp_obj) - 1;
#ifdef BWTREE_DEBUG
...
// NOISEPAGE_ASSERT(it == it2);
   ...
   */
  NO_ASAN NodeID NavigateInnerNode(Context *context_p) {
    ...
    while (1) {
      ...
      switch (type) {
        case NodeType::InnerType: {
          ...
          INDEX_LOG_TRACE("Found child in inner node; child ID = %" PRIu64 "", target_id);
        ...
        case NodeType::InnerInsertType: {
          ...
          if (KeyCmpGreaterEqual(search_key, insert_item.first)) {
            if ((next_item.second == INVALID_NODE_ID) || (KeyCmpLess(search_key, next_item.first))) {
              INDEX_LOG_TRACE("Find target ID = %" PRIu64 " in insert delta", insert_item.second);
        ...
        case NodeType::InnerDeleteType: {
          ...
          if ((delete_node_p->GetLowKeyNodeID() == prev_item.second) ||
            ...
            // then +Inf
            if ((next_item.second == INVALID_NODE_ID) || (KeyCmpLess(search_key, next_item.first))) {
              INDEX_LOG_TRACE("Find target ID = %" PRIu64 " in delete delta", prev_item.second);
        ...
        case NodeType::InnerMergeType: {
          ...
          if (KeyCmpGreaterEqual(search_key, merge_key)) {
            INDEX_LOG_TRACE("Take merge right branch (ID = %" PRIu64 ")", snapshot_p->node_id);

            node_p = merge_node_p->right_merge_p;
          } else {
            INDEX_LOG_TRACE("Take merge left branch (ID = %" PRIu64 ")", snapshot_p->node_id);
   ...
   */
  NO_ASAN NodeID NavigateInnerNodeBI(Context *context_p) {
    ...
    while (1) {
      ...
      switch (type) {
        ...
        case NodeType::InnerInsertType: {
          ...
          if ((next_item.second == INVALID_NODE_ID) || (KeyCmpLess(search_key, next_item.first))) {
            ...
            if (KeyCmpGreater(search_key, insert_item.first)) {
              INDEX_LOG_TRACE("Find target ID = %" PRIu64 " in insert delta (BI)", insert_item.second);
        ...
        case NodeType::InnerDeleteType: {
          ...
          // * NOTE: DO NOT PROCEED IF IT IS "==" RELATION
          // *********************************************
          if ((delete_node_p->GetLowKeyNodeID() == prev_item.second) || (KeyCmpGreater(search_key, prev_item.first))) {
            if ((next_item.second == INVALID_NODE_ID) || (KeyCmpLess(search_key, next_item.first))) {
              INDEX_LOG_TRACE("Find target ID = %" PRIu64 " in delete delta (BI)", prev_item.second);
        ...
        case NodeType::InnerMergeType: {
          ...
          if (KeyCmpGreater(search_key, merge_key)) {
            INDEX_LOG_TRACE("Take merge right branch (ID = %" PRIu64 ") for BI", snapshot_p->node_id);

            node_p = merge_node_p->right_merge_p;
          } else {
            INDEX_LOG_TRACE("Take merge left branch (ID = %" PRIu64 ") for BI", snapshot_p->node_id);
  ...
  NO_ASAN InnerNode *CollectAllSepsOnInner(NodeSnapshot *snapshot_p, int p_depth = 0) {
    ...
    auto *inner_node_p = reinterpret_cast<InnerNode *>(
        ElasticNode<KeyNodeIDPair>::Get(node_p->GetItemCount(), NodeType::InnerType, p_depth, node_p->GetItemCount(),
    ...
    // and values deleted
    CollectAllSepsOnInnerRecursive(node_p, node_p->GetLowKeyNodeID(), sss, inner_node_p);
      ...
      switch (type) {
        case NodeType::InnerType: {
          ...
          if (high_key_pair.second == INVALID_NODE_ID) {
            ...
            // consistent
            copy_end_it = std::lower_bound(inner_node_p->Begin() + 1, inner_node_p->End(),
          ...
          if (high_key_pair.second != INVALID_NODE_ID) {
            ...
            while (sss_end_it >= sss.GetBegin()) {
              if (static_cast<bool>(key_cmp_obj((*sss_end_it)->item.first, high_key_pair.first))) {
                break;
              }
          ...
          while (true) {
            ...
            if (sss_end_flag && array_end_flag) {
              // Both are drained
              break;
            }
            if (sss_end_flag) {
            ...
            if (static_cast<bool>(key_cmp_obj(copy_start_it->first, sss.GetFront()->item.first))) {
              // If array element is less than data node list element
              new_inner_node_p->PushBack(*copy_start_it);

              copy_start_it++;
            } else if (static_cast<bool>(key_cmp_obj(sss.GetFront()->item.first, copy_start_it->first))) {
              NodeType data_node_type = (sss.GetFront())->GetType();
        ...
        case NodeType::InnerMergeType: {
          ...
          CollectAllSepsOnInnerRecursive(merge_node_p->child_node_p, low_key_node_id, sss, new_inner_node_p);

          CollectAllSepsOnInnerRecursive(merge_node_p->right_merge_p, low_key_node_id, sss, new_inner_node_p);
   ...
   * map insert conflict
   */
  NO_ASAN void NavigateLeafNode(Context *context_p, std::vector<ValueType> &value_list) {
    // This will go to the right sibling until we have seen
    // a node whose range match the search key
    NavigateSiblingChain(context_p);
    ...
    while (1) {
      ...
      switch (type) {
        case NodeType::LeafType: {
          ...
          auto end_it = ((end_index == -1) ? leaf_node_p->End() : leaf_node_p->Begin() + end_index);

          // Here we know the search key < high key of current node
          // NOTE: We only compare keys here, so it will get to the first
          // element >= search key
          auto copy_start_it =
              std::lower_bound(start_it, end_it, std::make_pair(search_key, ValueType{}), key_value_pair_cmp_obj);

          // If there is something to copy
          while ((copy_start_it != leaf_node_p->End()) && (KeyCmpEqual(search_key, copy_start_it->first))) {
            // If the value has not been deleted then just insert
            // Note that here we use ValueSet, so need to extract value from
            // the key value pair
            if (!deleted_set.Exists(copy_start_it->second)) {
              if (!present_set.Exists(copy_start_it->second)) {
        ...
        case NodeType::LeafInsertType: {
          ...
          if (KeyCmpEqual(search_key, insert_node_p->item.first)) {
            if (!deleted_set.Exists(insert_node_p->item.second)) {
              ...
              if (!present_set.Exists(insert_node_p->item.second)) {
        ...
        case NodeType::LeafMergeType: {
          ...
          if (KeyCmpGreaterEqual(search_key, merge_node_p->delete_item.first)) {
            ...
            INDEX_LOG_TRACE("Take leaf merge left branch");
    ...
    while (1) {
      ...
      switch (type) {
        case NodeType::LeafType: {
          ...
          // NOTE: We only compare keys here, so it will get to the first
          // element >= search key
          auto scan_start_it = std::lower_bound(leaf_node_p->Begin(), leaf_node_p->End(),
                                                std::make_pair(search_key, ValueType{}), key_value_pair_cmp_obj);

          // Search all values with the search key
          while ((scan_start_it != leaf_node_p->End()) && (KeyCmpEqual(scan_start_it->first, search_key))) {
            ...
            if (unique_key || ValueCmpEqual(scan_start_it->second, search_value)) {
        ...
        case NodeType::LeafDeleteType: {
          ...
          if (KeyCmpEqual(search_key, delete_node_p->item.first)) {
            if (unique_key || ValueCmpEqual(delete_node_p->item.second, search_value)) {
          ...
          while ((copy_start_it != leaf_node_p->End()) && (KeyCmpEqual(search_key, copy_start_it->first))) {
            if (!deleted_set.Exists(copy_start_it->second)) {
              if (!present_set.Exists(copy_start_it->second)) {
                ...
                if (predicate(copy_start_it->second)) {
                  *predicate_satisfied = true;

                  return nullptr;
                }
                if (static_cast<bool>(value_eq_obj(value, copy_start_it->second))) {
          ...
          if (KeyCmpEqual(search_key, insert_node_p->item.first)) {
            if (!deleted_set.Exists(insert_node_p->item.second)) {
              if (!present_set.Exists(insert_node_p->item.second)) {
                ...
                if (predicate(insert_node_p->item.second)) {
                  *predicate_satisfied = true;

                  // Could return here since we know the predicate is satisfied
                  return nullptr;
                }
                if (static_cast<bool>(value_eq_obj(value, insert_node_p->item.second))) {
  ...
  NO_ASAN LeafNode *CollectAllValuesOnLeaf(NodeSnapshot *snapshot_p, LeafNode *leaf_node_p = nullptr) {
    ...
    // Prepare new node
    /////////////////////////////////////////////////////////////////
    // std::cout << "item : " << node_p->GetItemCount() << std::endl; 
    if (leaf_node_p == nullptr) {
      leaf_node_p = reinterpret_cast<LeafNode *>(
          ElasticNode<KeyValuePair>::Get(node_p->GetItemCount(), NodeType::LeafType, 0, node_p->GetItemCount(),
                                         node_p->GetLowKeyPair(), node_p->GetHighKeyPair()));
    }
    ...
    // key value pairs
    KeyValuePairBloomFilter delta_set{delta_set_data_p, key_value_pair_eq_obj, key_value_pair_hash_obj};
    ...
    auto f1 = [this](const LeafDataNode *ldn1, const LeafDataNode *ldn2) {
      ...
      if (this->key_eq_obj(ldn1->item.first, ldn2->item.first)) {
    ...
    CollectAllValuesOnLeafRecursive(node_p, sss, delta_set, leaf_node_p);
      // std::cout << "leaf : " << leaf_node_p->GetSize() << std::endl;
      // std::cout << "node : " << node_p->GetItemCount() << std::endl;
      // std::cout << "nodeid : " << snapshot_p->node_id << std::endl;
      ...
      switch (type) {
        ...
        case NodeType::LeafType: {
          ...
          if ((high_key_pair.second == INVALID_NODE_ID)) {
            ...
            copy_end_it = std::lower_bound(leaf_node_p->Begin(), leaf_node_p->End(),
                                           // It only compares key so we
                                           // just use high key pair
                                           std::make_pair(high_key_pair.first, ValueType{}), key_value_pair_cmp_obj);
          }

          // This is the index of the copy end it
          auto copy_end_index = static_cast<int>(copy_end_it - leaf_node_p->Begin());
          ...
          new_leaf_node_p->PushBack(leaf_node_p->Begin() + copy_start_index, leaf_node_p->Begin() + copy_end_index);
        ...
        case NodeType::LeafMergeType: {
          ...
          CollectAllValuesOnLeafRecursive(merge_node_p->right_merge_p, sss, delta_set, new_leaf_node_p);
   ...
   * decisions
   */
  NO_ASAN void JumpToLeftSibling(Context *context_p) {
    ...
    NodeID left_sibling_id = FindLeftSibling(snapshot_p->node_p->GetLowKey(), parent_snapshot_p);
    ...
    if (context_p->abort_flag) {
      INDEX_LOG_TRACE("JumpToLeftSibling()'s call to JumpToNodeID() ABORT");
   ...
   */
  NO_ASAN void LoadNodeID(NodeID node_id, Context *context_p) {
    INDEX_LOG_TRACE("Loading NodeID = %" PRIu64 "", node_id);
   ...
   */
  NO_ASAN void JumpToNodeID(NodeID node_id, Context *context_p) {
    INDEX_LOG_TRACE("Jumping to node ID = %" PRIu64 "", node_id);
   ...
   */
  NO_ASAN inline void FinishPartialSMOReadOptimized(Context *context_p) {
  ...
  before_switch:
    switch (snapshot_p->node_p->GetType()) {
      ...
      case NodeType::LeafRemoveType:
      case NodeType::InnerRemoveType: {
        INDEX_LOG_TRACE("Observed remove node; abort");
   ...
   */
  NO_ASAN inline void LoadNodeIDReadOptimized(NodeID node_id, Context *context_p) {
    INDEX_LOG_TRACE("Loading NodeID (RO) = %" PRIu64 "", node_id);
  ...
  retry_traverse:
    ...
    INDEX_LOG_TRACE("Successfully loading root node ID for BI");
    ...
    while (1) {
      ...
      LoadNodeID(child_node_id, context_p);
      if (context_p->abort_flag) {
        INDEX_LOG_TRACE("LoadNodeID aborted (BI). ABORT");
        goto abort_traverse;
      }

      NodeSnapshot *snapshot_p = GetLatestNodeSnapshot(context_p);
      if (snapshot_p->IsLeaf()) {
        INDEX_LOG_TRACE("The next node is a leaf (BI)");

        // After reaching leaf level just traverse the sibling chain
        // and stop before the search key
        NavigateSiblingChainBI(context_p);
        if (context_p->abort_flag) {
          INDEX_LOG_TRACE("NavigateSiblingChainBI() inside TraverseBI() aborts");
  ...
  retry_traverse:
    ...
    if (context_p->abort_flag) {
      INDEX_LOG_TRACE("LoadNodeID aborted on loading root (RO)");

      goto abort_traverse;
    }

    INDEX_LOG_TRACE("Successfully loading root node ID (RO)");
    ...
    while (1) {
      ...
      // if there is a split delta and the key is >= split key
      if (context_p->abort_flag) {
        INDEX_LOG_TRACE("Navigate Inner Node abort (RO)");
      ...
      if (context_p->abort_flag) {
        INDEX_LOG_TRACE("LoadNodeID aborted (RO). ABORT");
      ...
      if (snapshot_p->IsLeaf()) {
        ...
        INDEX_LOG_TRACE("Found leaf node (RO). Abort count = %d, level = %d", context_p->abort_counter,
                        context_p->current_level);
  ...
  NO_ASAN inline bool PostInnerInsertNode(Context *context_p, const KeyNodeIDPair &insert_item,
    ...
    bool ret = InstallNodeToReplace(parent_snapshot_p->node_id, insert_node_p, parent_snapshot_p->node_p);

    if (ret) {
      INDEX_LOG_TRACE("Index term insert (from %" PRIu64 " to %" PRIu64 ") delta CAS succeeds",
    ...
    INDEX_LOG_TRACE("Index term insert (from %" PRIu64 " to %" PRIu64
                    ") delta CAS failed. "
                    "ABORT",
                    GetLatestNodeSnapshot(context_p)->node_id, insert_item.second);
    ...
    installnode_failure_cnt.fetch_add(1);
  ...
  NO_ASAN inline bool PostInnerDeleteNode(Context *context_p, const KeyNodeIDPair &delete_item,
    ...
    bool ret = InstallNodeToReplace(parent_snapshot_p->node_id, delete_node_p, parent_snapshot_p->node_p);
    ...
    if (ret) {
      INDEX_LOG_TRACE("Index term delete delta installed, ID = %" PRIu64 "; ABORT", parent_snapshot_p->node_id);
    ...
    INDEX_LOG_TRACE("Index term delete delta install failed. ABORT");
   ...
   */
  NO_ASAN void FinishPartialSMO(Context *context_p) {
    // Note: If the top of the path list changes then this pointer
    // must also be updated
    NodeSnapshot *snapshot_p = GetLatestNodeSnapshot(context_p);
  ...
  before_switch:
    switch (snapshot_p->node_p->GetType()) {
      case NodeType::InnerAbortType: {
        INDEX_LOG_TRACE("Observed Inner Abort Node; ABORT");
      ...
      case NodeType::InnerRemoveType: {
        ...
        if (context_p->abort_flag) {
          INDEX_LOG_TRACE("Jump to left sibling in Remove help along ABORT");
        ...
        if (left_snapshot_p->IsLeaf()) {
          ret = PostLeafMergeNode(left_snapshot_p, &merge_key, merge_right_branch, deleted_node_id, &merge_node_p);
        } else {
          ret = PostInnerMergeNode(left_snapshot_p, &merge_key, merge_right_branch, deleted_node_id, &merge_node_p);
        }
        ...
        if (ret) {
          ...
          SMO_table[left_snapshot_p->node_id] = true;

          // merge_node_p is set as the newest merge node above
          // after this point
          // Also snapshot_p has been to left_snapshot_p above
        } else {
          INDEX_LOG_TRACE("Merge delta CAS fails. ABORT");

          context_p->abort_flag = true;

          finishSMO_failure_cnt.fetch_add(1);
      ...
      case NodeType::InnerMergeType:
      case NodeType::LeafMergeType: {
        INDEX_LOG_TRACE("Helping along merge delta");
        ...
        PostInnerDeleteNode(context_p, *delete_item_p,
      ...
      case NodeType::InnerSplitType:
      case NodeType::LeafSplitType: {
        INDEX_LOG_TRACE("Helping along split node");
        ...
        if (context_p->IsOnRootNode()) {
          ...
          InnerNode *inner_node_p = reinterpret_cast<InnerNode *>(ElasticNode<KeyNodeIDPair>::Get(
              2, NodeType::InnerType, 0, 2, first_item, std::make_pair(KeyType{}, INVALID_NODE_ID)));
          ...
          if (ret) {
            INDEX_LOG_TRACE("Install root CAS succeeds");
          ...
          INDEX_LOG_TRACE("Install root CAS failed. ABORT");
          ...
          if ((parent_snapshot_p->node_p->GetNextNodeID() != INVALID_NODE_ID) &&
              (KeyCmpGreaterEqual(insert_item_p->first, parent_snapshot_p->node_p->GetHighKey()))) {
            INDEX_LOG_TRACE(
          ...
          // Note: Even if this function aborts, since we return immediately
          // so do not have to test abort_flag here
          PostInnerInsertNode(context_p, *insert_item_p, *next_item_p, location);
  ...
  NO_ASAN inline void ConsolidateLeafNode(NodeSnapshot *snapshot_p) {
    ...
    // std::cout << "0" << std::endl;
    const BaseNode *recycle_node = GetRecycleNode(snapshot_p->node_id);
    LeafNode *leaf_node_p;

    if (recycle_node != nullptr) {
    // //   // std::cout << "if" << std::endl;
      leaf_node_p = CollectAllValuesOnLeafWithRecycle(snapshot_p, recycle_node);
    } else {
      // std::cout << "else" << std::endl;
      leaf_node_p = CollectAllValuesOnLeaf(snapshot_p);
    }
    ...
    if (ret) {
      // std::cout << "1" << std::endl;
      retry_table[snapshot_p->node_id] = nullptr;
      epoch_manager.AddGarbageNode(snapshot_p->node_p); 
      // std::cout << "2" << std::endl;
      if (recycle_node != nullptr)
        epoch_manager.AddGarbageNode(REFERENCE(recycle_node));

      snapshot_p->node_p = leaf_node_p;
    } else {
      // epoch_manager.AddGarbageNode(leaf_node_p);

      const BaseNode *recycle_node_p = SET_DEPTH_AT_NODE(leaf_node_p, snapshot_p->node_p->GetDepth());
      InstallNodeToRetry(snapshot_p->node_id, recycle_node_p, recycle_node);
      consol_failure_cnt.fetch_add(1);
    }
    // std::cout << "3" << std::endl;
  }

  NO_ASAN LeafNode *CollectAllValuesOnLeafWithRecycle(NodeSnapshot *snapshot_p, const BaseNode *recycle_node, LeafNode *leaf_node_p = nullptr) {
    NOISEPAGE_ASSERT(snapshot_p->IsLeaf(), "Must be a leaf node.");

    const BaseNode *node_p = snapshot_p->node_p;
    LeafNode *recycle_node_p = reinterpret_cast<LeafNode *>(REFERENCE(recycle_node));
    int depth_p = GET_DEPTH_FROM_NODE(recycle_node);
    /////////////////////////////////////////////////////////////////
    // Prepare new node
    /////////////////////////////////////////////////////////////////
    ...
    // This is the number of delta records inside the logical node
    // including merged delta chains
    int delta_change_num = node_p->GetDepth() - depth_p;
    // std::cout << "recycle:" << recycle_node->depth << std::endl;
    // std::cout << "delta num:" << node_p->GetDepth() << std::endl;

    if (delta_change_num <= 0) {
      return leaf_node_p;
    }
    ...
    // We collect all valid values in present_set
    // and deleted_set is just for bookkeeping
    CollectAllValuesOnLeafRecursiveWithRecycle(node_p, sss, delta_set, SET_DEPTH_AT_NODE(recycle_node, delta_change_num), leaf_node_p);
  ...
  template <typename T>  // To let the compiler deduce type of sss
  NO_ASAN void CollectAllValuesOnLeafRecursiveWithRecycle(const BaseNode *node_p, T &sss, KeyValuePairBloomFilter &delta_set,
                                               BaseNode *recycle_node, LeafNode *new_leaf_node_p) const {
    // The top node is used to derive high key
    // NOTE: Low key for Leaf node and its delta chain is nullptr
    const KeyNodeIDPair &high_key_pair = node_p->GetHighKeyPair();
    LeafNode *recycle_node_p = reinterpret_cast<LeafNode *>(REFERENCE(recycle_node));
    ...
    const auto *leaf_node_p = static_cast<const LeafNode *>(recycle_node_p);
  ...
  NO_ASAN inline void ConsolidateInnerNode(NodeSnapshot *snapshot_p) {
    ...
    if (recycle_node != nullptr) {
      inner_node_p = CollectAllSepsOnInnerWithRecycle(snapshot_p, recycle_node);
    } else {
    ...
    if (ret) {
      ...
      epoch_manager.AddGarbageNode(snapshot_p->node_p);
      if (recycle_node != nullptr)
        epoch_manager.AddGarbageNode(recycle_node);

      snapshot_p->node_p = inner_node_p;
    } else {
      epoch_manager.AddGarbageNode(inner_node_p);
      // RecycleNode *recycle_node_p = new RecycleNode{inner_node_p, snapshot_p->node_p->GetDepth(), false};

      const BaseNode *recycle_node_p = SET_DEPTH_AT_NODE(inner_node_p, snapshot_p->node_p->GetDepth());
      InstallNodeToRetry(snapshot_p->node_id, recycle_node_p, recycle_node);
      consol_failure_cnt.fetch_add(1);
    }
  }

  NO_ASAN InnerNode *CollectAllSepsOnInnerWithRecycle(NodeSnapshot *snapshot_p, const BaseNode *recycle_node_p) {
    // Note that in the recursive call node_p might change
    // but we should not change the metadata
    const BaseNode *node_p = snapshot_p->node_p;
    InnerNode *recycle_node = reinterpret_cast<InnerNode *>(REFERENCE(recycle_node));
    int depth_p = GET_DEPTH_FROM_NODE(recycle_node);

    // This is the number of insert + delete records which contributes
    // to the size of the bloom filter
    int delta_record_num = node_p->GetDepth() - depth_p;
    if (delta_record_num <= 0)
      return recycle_node;
    ...
    auto *inner_node_p = reinterpret_cast<InnerNode *>(REFERENCE(recycle_node_p));
  ...
  NO_ASAN void AdjustNodeSize(Context *context_p) {
    ...
    if (node_p->IsDeltaNode()) {
      // TODO(Ziqi): If we want to strictly enforce the size of any node
    ...
    if (snapshot_p->IsLeaf()) {
      ...
      if (node_size >= GetLeafNodeSizeUpperThreshold()) {
        ...
        adjustnode_failure_cnt.fetch_add(1);
      ...
      if (node_size >= GetInnerNodeSizeUpperThreshold()) {
        ...
        if (split_key_child_node_p->IsRemoveNode()) {
          INDEX_LOG_TRACE("Found a removed node on split key child. CONTINUE ");
        ...
        if (ret) {
          INDEX_LOG_TRACE("Inner split delta (from %" PRIu64 " to %" PRIu64
                          ") CAS succeeds."
                          " ABORT",
                          node_id, new_node_id);
        ...
        INDEX_LOG_TRACE("Inner split delta CAS fails");
        ...
        if (IsOnLeftMostChild(context_p)) {
          INDEX_LOG_TRACE("Left most inner node cannot be removed");
        ...
        INDEX_LOG_TRACE("Node size <= inner lower threshold. Remove");
        ...
        // (splitted, etc.)
        if (abort_node_ret) {
          INDEX_LOG_TRACE("Blocked parent node (current node is inner)");
        } else {
          INDEX_LOG_TRACE(
              "Unable to block parent node "
        ...
        bool ret = InstallNodeToReplace(node_id, remove_node_p, node_p);
        if (ret) {
          INDEX_LOG_TRACE("InnerRemoveNode CAS succeeds. ABORT");
        ...
        INDEX_LOG_TRACE("InnerRemoveNode CAS failed");
  ...
  NO_ASAN void RemoveAbortOnParent(NodeID parent_node_id, const BaseNode *abort_node_p,
    ...
    bool ret = InstallNodeToReplace(parent_node_id, abort_child_node_p, abort_node_p);
  ...
  NO_ASAN bool PostAbortOnParent(Context *context_p, NodeID *parent_node_id_p, const BaseNode **abort_node_p_p,
    ...
    *parent_node_id_p = parent_node_id;
    ...
    auto *abort_node_p = new InnerAbortNode{parent_node_p};

    bool ret = InstallNodeToReplace(parent_node_id, abort_node_p, parent_node_p);

    if (ret) {
      INDEX_LOG_TRACE("Inner Abort node CAS succeeds");
      ...
      *abort_node_p_p = abort_node_p;
      SMO_table[parent_node_id] = true;
    } else {
      INDEX_LOG_TRACE("Inner Abort node CAS failed");
    ...
    while (1) {
      ...
      switch (type) {
        case NodeType::InnerType: {
          ...
          auto it1 = std::upper_bound(inner_node_p->Begin() + 1, end_it, std::make_pair(search_key, INVALID_NODE_ID),
                                      key_node_id_pair_cmp_obj) -
                     1;
        ...
        case NodeType::InnerMergeType: {
          ...
          if (ret) {
            epoch_manager.AddGarbageNode(snapshot_p->node_p);
            SMO_table[snapshot_p->node_id] = true;
            snapshot_p->node_p = inner_node_p;
          } else {
  ...
  NO_ASAN bool PostLeafMergeNode(const NodeSnapshot *snapshot_p, const KeyType *merge_key_p,
    ...
    if (!ret) {
      ...
      // SMO_table[deleted_node_id] = true;
      *node_p_p = merge_node_p;
      SMO_table[node_id] = true;
    }
  ...
  NO_ASAN const KeyValuePair *TraverseFromParent(Context *context_p, const ValueType *value_p, std::pair<int, bool> *index_pair_p,
                                       bool unique_key = false) {
    // For value collection it always returns nullptr
    const KeyValuePair *found_pair_p = nullptr;
  ...
  retry_traverse:
    ...
    context_p->current_snapshot.node_id = INVALID_NODE_ID;
    // std::cout << "0" << std::endl;
    // std::cout << "parent : " << parent_node_id << std::endl;
    // std::cout << "child : " << context_p->current_snapshot.node_id << std::endl;
    LoadNodeID(parent_node_id, context_p);
...
retry:
      ...
      if (!SMO_table[node_id]) {
        TraverseFromParent(&context, &value, &index_pair, unique_key);
...
abort:
      ...
      insert_failure_cnt.fetch_add(1);
      
      // We reach here only because CAS failed
      INDEX_LOG_TRACE("Retry installing leaf insert delta from the root");
    }

    epoch_manager.LeaveEpoch(epoch_node_p);

    index_size.fetch_add(1);
    // std::cout << "indexsize : " << index_size << std::endl;
    return true;
  }
...
retry:
      ...
      if (!SMO_table[node_id]) {
        TraverseFromParent(&context, &value, &index_pair);
...
abort:
      ...
      INDEX_LOG_TRACE("Retry installing leaf insert (cond.) delta from the root");
    }
...
abort:
      ...
      delete_failure_cnt.fetch_add(1);

      // We reach here only because CAS failed
      INDEX_LOG_TRACE("Retry installing leaf delete delta from the root");
    }
  ...
  NO_ASAN ValueSet GetValue(const KeyType &search_key) {
    ...
    ValueSet value_set{value_list.begin(), value_list.end(), 10, value_hash_obj, value_eq_obj};
   ...
   * counter.
   */
  NO_ASAN bool NeedGarbageCollection() { return true; }
 ...
 public:
  ...
  std::atomic<const BaseNode *> *retry_table;
  std::atomic<bool> *SMO_table;
  ...
  std::atomic<uint64_t> consol_failure_cnt;
  std::atomic<uint64_t> insert_failure_cnt;
  std::atomic<uint64_t> delete_failure_cnt;
  std::atomic<uint64_t> finishSMO_failure_cnt;
  std::atomic<uint64_t> installnode_failure_cnt;
  std::atomic<uint64_t> adjustnode_failure_cnt;
...
// maintains its own memory
     ...
     */
    NO_ASAN EpochManager(BwTree *p_tree_p) : tree_p{p_tree_p} {
    ...
    NO_ASAN ~EpochManager() {
      ...
      if (thread_p != nullptr) {
        INDEX_LOG_TRACE("Waiting for thread");
      ...
      INDEX_LOG_TRACE("Clearing the epoch in ~EpochManager() ...");
      ...
      if (head_epoch_p != nullptr) {
        INDEX_LOG_DEBUG("ERROR: After cleanup there is still epoch left");
        INDEX_LOG_DEBUG("DUMP");

        for (EpochNode *epoch_node_p = head_epoch_p; epoch_node_p != nullptr; epoch_node_p = epoch_node_p->next_p) {
          INDEX_LOG_DEBUG("Active thread count: %d", epoch_node_p->active_thread_count.load());
          epoch_node_p->active_thread_count = 0;
        }

        INDEX_LOG_DEBUG("RETRY CLEANING...");
        ClearEpoch();
      }
...
#ifdef BWTREE_DEBUG
      INDEX_LOG_TRACE("Stat: Freed %" PRIu64 " nodes and %" PRIu64 " NodeID by epoch manager", freed_count,
                      freed_id_count);

      INDEX_LOG_TRACE("      Epoch created = %" PRIu64 "; epoch freed = %" PRIu64 "", epoch_created, epoch_freed);

      INDEX_LOG_TRACE("      Epoch join = %" PRIu64 "; epoch leave = %" PRIu64 "", epoch_join.load(),
                      epoch_leave.load());
#endif
     ...
     */
    NO_ASAN void CreateNewEpoch() {
      INDEX_LOG_TRACE("Creating new epoch...");

      auto *epoch_node_p = new EpochNode{};
    ...
    NO_ASAN void AddGarbageNode(const BaseNode *node_p) {
      ...
      auto *garbage_node_p = new GarbageNode;
      ...
      while (1) {
        ...
        INDEX_LOG_TRACE("Add garbage node CAS failed. Retry");
      }  // while 1
    }
    ...
    NO_ASAN void ClearEpoch() {
      ...
      while (1) {
        ...
        if (head_epoch_p == current_epoch_p) {
          INDEX_LOG_TRACE("Current epoch is head epoch. Do not clean");
        ...
        if (active_thread_count != 0) {
          INDEX_LOG_TRACE("Head epoch is not empty. Return");
    ...
    NO_ASAN void ThreadFunc() {
      ...
      INDEX_LOG_TRACE("exit flag is true; thread return");
    }
   ...
   * is a valid state.
   */
  NO_ASAN ForwardIterator Begin() { return ForwardIterator{this}; }
   ...
   */
  NO_ASAN ForwardIterator NullIterator() { return ForwardIterator{}; }
  ...
  class IteratorContext {
   private:
     ...
     */
    NO_ASAN IteratorContext(BwTree *p_tree_p) : tree_p{p_tree_p}, ref_count{0UL} {}
   ...
   public:
     ...
     */
    NO_ASAN inline LeafNode *GetLeafNode() { return &leaf_node_p[0]; }

    /*
     * GetTree() - Returns a tree instance
     */
    NO_ASAN inline BwTree *GetTree() { return tree_p; }
     ...
     */
    NO_ASAN inline size_t GetRefCount() { return ref_count; }
    ...
    NO_ASAN inline static IteratorContext *Get(BwTree *p_tree_p, const BaseNode *node_p) {
      ...
      new (ic_p->GetLeafNode()) ElasticNode<KeyValuePair>{node_p->GetType(), node_p->GetDepth(), node_p->GetItemCount(),
                                                          node_p->GetLowKeyPair(), node_p->GetHighKeyPair()};
  ...
  class ForwardIterator {
   ...
   public:
     ...
     */
    NO_ASAN ForwardIterator() : ic_p{nullptr}, kv_p{nullptr} {}
     ...
     * know it is there
     */
    NO_ASAN ForwardIterator(BwTree *p_tree_p) {
    ...
    NO_ASAN bool IsEnd() const {
      ...
      return (ic_p->GetLeafNode()->GetNextNodeID() == INVALID_NODE_ID) && (ic_p->GetLeafNode()->End() == kv_p);
    }
    ...
    NO_ASAN bool IsBegin() const {
      ...
      return (ic_p->GetLeafNode()->GetLowKeyPair().second == INVALID_NODE_ID) && (ic_p->GetLeafNode()->Begin() == kv_p);
    }
     ...
     */
    NO_ASAN inline ForwardIterator &operator++() {
      if (IsEnd()) {
        return *this;
      }
     ...
     */
    NO_ASAN inline ForwardIterator operator++(int) {
      if (IsEnd()) {
        return *this;
      }
    ...
    NO_ASAN void LowerBound(BwTree *p_tree_p, const KeyType *start_key_p) {
      ...
      while (1) {
        ...
        //      been merged
        kv_p = std::lower_bound(ic_p->GetLeafNode()->Begin(), ic_p->GetLeafNode()->End(),
                                std::make_pair(start_key, ValueType{}), p_tree_p->key_value_pair_cmp_obj);
    ...
    NO_ASAN void MoveBackByOne() {
      ...
      while (1) {
        ...
        //        and kv_p-- is REnd()
        kv_p = std::lower_bound(ic_p->GetLeafNode()->Begin(), ic_p->GetLeafNode()->End(),
                                std::make_pair(low_key, ValueType{}), tree_p->key_value_pair_cmp_obj) -
               1;
    ...
    NO_ASAN inline void MoveAheadByOne() {
      ...
      if (kv_p == ic_p->GetLeafNode()->End()) {
        ...
        LowerBound(ic_p->GetTree(), &ic_p->GetLeafNode()->GetHighKeyPair().first);
      }
    }
  };  // ForwardIterator
